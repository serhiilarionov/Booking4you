 ...
 */
@Injectable()
export class LogsApi extends BaseLoopBackApi {
  ...
  public create(data: any = undefined) {
    ...
    return result.map((instance: Logs) => new Logs(instance));
  }
  ...
  public find(filter: LoopBackFilter = undefined) {
    ...
    if (filter) urlParams.filter = filter;
    let result = this.request(method, url, routeParams, urlParams, postBody);
    return result.map((instances: Array<Logs>) =>
        instances.map((instance: Logs) => new Logs(instance))
    );
  }
