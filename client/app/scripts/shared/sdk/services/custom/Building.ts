 ...
 */
@Injectable()
export class BuildingApi extends BaseLoopBackApi {
  ...
  public create(data: any = undefined) {
    ...
    return result.map((instance: Building) => new Building(instance));
  }
  ...
  public find(filter: LoopBackFilter = undefined) {
    ...
    if (filter) urlParams.filter = filter;
    let result = this.request(method, url, routeParams, urlParams, postBody);
    return result.map((instances: Array<Building>) =>
        instances.map((instance: Building) => new Building(instance))
    );
  }
