 ...
 */
@Injectable()
export class NotifyApi extends BaseLoopBackApi {
  ...
  public create(data: any = undefined) {
    ...
    let urlParams: any = {};
    let result = this.request(method, url, routeParams, urlParams, postBody);
    return result.map((instance: Notify) => new Notify(instance));
  }
  ...
  public find(filter: LoopBackFilter = undefined) {
    ...
    if (filter) urlParams.filter = filter;
    let result = this.request(method, url, routeParams, urlParams, postBody);
    return result.map((instances: Array<Notify>) =>
        instances.map((instance: Notify) => new Notify(instance))
    );
  }
