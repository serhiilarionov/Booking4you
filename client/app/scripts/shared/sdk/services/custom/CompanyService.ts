 ...
 */
@Injectable()
export class CompanyServiceApi extends BaseLoopBackApi {
  ...
  public findByIdBookings(id: any, fk: any) {
    ...
    let urlParams: any = {};
    let result = this.request(method, url, routeParams, urlParams, postBody);
    return result.map((instance: CompanyService) => new CompanyService(instance));
  }
  ...
  public find(filter: LoopBackFilter = undefined) {
    ...
    if (filter) urlParams.filter = filter;
    let result = this.request(method, url, routeParams, urlParams, postBody);
    return result.map((instances: Array<CompanyService>) =>
        instances.map((instance: CompanyService) => new CompanyService(instance))
    );
  }
