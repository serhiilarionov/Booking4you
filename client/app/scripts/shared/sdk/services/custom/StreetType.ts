 ...
 */
@Injectable()
export class StreetTypeApi extends BaseLoopBackApi {
  ...
  public create(data: any = undefined) {
    ...
    return result.map((instance: StreetType) => new StreetType(instance));
  }
  ...
  public find(filter: LoopBackFilter = undefined) {
    ...
    if (filter) urlParams.filter = filter;
    let result = this.request(method, url, routeParams, urlParams, postBody);
    return result.map((instances: Array<StreetType>) =>
        instances.map((instance: StreetType) => new StreetType(instance))
    );
  }
