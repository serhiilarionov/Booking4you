...
export class DistrictApi extends BaseLoopBackApi {
  ...
  public findByIdStreets(id: any, fk: any) {
    ...
    return result.map((instance: District) => new District(instance));
  }
  ...
  public find(filter: LoopBackFilter = undefined) {
    ...
    if (filter) urlParams.filter = filter;
    let result = this.request(method, url, routeParams, urlParams, postBody);
    return result.map((instances: Array<District>) =>
        instances.map((instance: District) => new District(instance))
    );
  }
