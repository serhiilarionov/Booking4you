 ...
 */
@Injectable()
export class CompanyDetailApi extends BaseLoopBackApi {
  ...
  public create(data: any = undefined) {
    ...
    return result.map((instance: CompanyDetail) => new CompanyDetail(instance));
  }
  ...
  public find(filter: LoopBackFilter = undefined) {
    ...
    if (filter) urlParams.filter = filter;
    let result = this.request(method, url, routeParams, urlParams, postBody);
    return result.map((instances: Array<CompanyDetail>) =>
        instances.map((instance: CompanyDetail) => new CompanyDetail(instance))
    );
  }
