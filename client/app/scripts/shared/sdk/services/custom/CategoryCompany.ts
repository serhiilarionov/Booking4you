 ...
 */
@Injectable()
export class CategoryCompanyApi extends BaseLoopBackApi {
  ...
  public create(data: any = undefined) {
    ...
    let urlParams: any = {};
    let result = this.request(method, url, routeParams, urlParams, postBody);
    return result.map((instance: CategoryCompany) => new CategoryCompany(instance));
  }
  ...
  public find(filter: LoopBackFilter = undefined) {
    ...
    if (filter) urlParams.filter = filter;
    let result = this.request(method, url, routeParams, urlParams, postBody);
    return result.map((instances: Array<CategoryCompany>) =>
        instances.map((instance: CategoryCompany) => new CategoryCompany(instance))
    );
  }
